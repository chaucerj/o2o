...
import com.chaucer.o2o.exceptions.ShopCategoryOperationException;
...
public class ShopCategoryServiceImpl implements ShopCategoryService {
	...
	private static Logger logger = LoggerFactory
			.getLogger(ShopCategoryServiceImpl.class);
	...
	public List<ShopCategory> getShopCategoryList(
		...
		if (shopCategoryCondition == null) {
			...
			key = key + "_allfirstlevel";
				...
				&& shopCategoryCondition.getParent()
						.getShopCategoryId() != null) {
			// 若parentId为非空，则列出该parentId下的所有子类别
			key = key + "_parent"
					+ shopCategoryCondition.getParent().getShopCategoryId();
		} else if (shopCategoryCondition != null) {
		...
		if (!jedisKeys.exists(key)) {
			...
			JavaType javaType = mapper.getTypeFactory().constructParametricType(
					ArrayList.class, ShopCategory.class);
			try {
